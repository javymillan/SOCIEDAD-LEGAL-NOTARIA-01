import { Document, Packer, Paragraph, TextRun, AlignmentType, HeadingLevel, convertInchesToTwip, Table, TableRow, TableCell, BorderStyle, WidthType, ShadingType } from 'docx';
...
function createSocioCard(index) {
    ...
    const removeBtnHtml = index > 0 ? `
        <button type="button" class="btn-remove" onclick="removeSocio(${index})">
...
function generatePreview() {
    ...
    preview.innerHTML = `
        ...
        <div class="document-preview-paper" contenteditable="true" style="font-family: Arial, sans-serif; font-size: 9pt;">
            <p style="text-align: center; font-weight: bold;">ESCRITURA PÚBLICA NÚMERO ${hl(data.numeroEscritura)}</p>
            <p style="text-align: center; font-weight: bold;">VOLUMEN ${hl(data.volumen)}</p>
            ...
            <div style="margin-top: 3rem; padding-top: 2rem; border-top: 2px solid var(--gray-300);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4rem; text-align: center;">
                    ...
                    <div>
                        <div style="border-bottom: 1px solid black; margin-bottom: 0.5rem; width: 80%; margin-left: auto; margin-right: auto;"></div>
                        <strong>${hl(data.notario.split(',')[0].trim())}</strong><br>
...
function generateFirmas(data) {
    ...
    paragraphs.push(
        ...
        new Paragraph({
            children: [
                new TextRun({
                    text: data.notario.split(',')[0].trim(),
                    size: 18 }),
            ],
            alignment: AlignmentType.CENTER,
            spacing: { after: 100 } }),
